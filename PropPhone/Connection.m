...
@property(unsafe_unretained, nonatomic) int port;
@property(unsafe_unretained, nonatomic) CFSocketNativeHandle connectedSocketHandle;
...
- (void)dealloc {
    ...
    [self clean];
     ...
     self.netService = nil;
     self.host = nil;
         //self.delegate = nil; // self.delegate was originally set to retain...?
         */
}
...
- (id)initWithNetService:(NSNetService*)_netService {
    ...
    if ( _netService.hostName != nil ) {
        NSLog(@"Connection.m _netService.hostName: %@", _netService.hostName);
        return [self initWithHostAddress:_netService.hostName andPort:_netService.port];
    }
    
    // NSLog(@"Connection.m _netService==nil?:%@", _netService == nil ? @"YES" : @"NO");
...
- (BOOL)setupSocketStreams {
    // printf("setupSocketStreams Started\n");
    // Make sure streams were created correctly
    if ( readStream == nil || writeStream == nil ) {
      [self close];
        printf("readStream or writeStream == nil\n");
      return NO;
    }
    ...
    if ( ! CFReadStreamOpen(readStream) || ! CFWriteStreamOpen(writeStream)) {
      [self close];
      printf("couldn't open readStream or writeStream\n");
      return NO;
    }
    // printf("Sockets are successfully setup\n");
    return YES;
}
